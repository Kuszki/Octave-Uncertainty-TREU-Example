...
for j = 1 : length(amps)
v_obl = v_obl + amps(j)^2 * (1 - (1+(theta^2)*(omegas(j)^2))^(-k/2) * cos(k*atan(theta*omegas(j))));
v_obl = v_obl + amps(j)^2 * (1 - (sin(omegas(j) * tdif_max) - sin(omegas(j) * tdif_min)) / (omegas(j) * (tdif_max - tdif_min)));
end

for i = 1 : nsam

	x1 = x0 + tdif(i) + (tdif_max-tdif_min)*rand(1, step) + tdif_min;
	y0 = y1 = zeros(1, step);
...
printf('\\textbf{Case 2)} $\\Delta t_{s} \\sim G(\\num{%0.2f}, \\qty{%0.1e}{s})$, $\\Delta t_{i} \\sim U(\\qty{%0.1e}{s}, \\qty{%0.1e}{s})$, $\\delta = \\qty{%0.2f}{\\percent}$ \\vskip6pt \\includegraphics{Pictures/hist_3_gamma_uniform}\n', k, theta, tdif_min, tdif_max, v_diff)

print("hist_3_gamma_uniform.svg");
