...
tdif_std = 2.15e-5;
tdif_mean = 5 * tdif_std;

tdif = tdif_mean + gen_randn(nsam, tdif_std, 's');
tdif(nsam) = mean(tdif);

assert(min(tdif) > 0, 'negative values')
...
printf('\\textbf{Case 2)} $\\Delta t_{s} \\sim N(\\qty{%0.1e}{s}, \\qty{%0.1e}{s^2})$, $\\Delta t_{i} = \\qty{0}{s}$, $\\delta = \\qty{%0.2f}{\\percent}$ \\vskip6pt \\includegraphics{Pictures/hist_1_normal}\n',  tdif_mean, tdif_std^2, v_diff)

print("hist_1_normal.svg");
