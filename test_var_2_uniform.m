...
for i = 1 : nsam
	...
	x1 = x0 + (tdif_max-tdif_min)*rand(1, step) + tdif_min;
	y0 = y1 = zeros(1, step);
...
xt = 4.5*sqrt(v_rea);
hist(err(:), 300, 100, "facecolor", fcolor, "edgecolor", ecolor)
...
printf('\\textbf{Case 1)} $\\Delta t_{i} \\sim U(\\qty{%0.1e}{s}, \\qty{%0.1e}{s})$, $\\Delta t_{s} = \\qty{0}{s}$, $\\delta = \\qty{%0.2f}{\\percent}$ \\vskip6pt \\includegraphics{Pictures/hist_2_uniform}\n',  tdif_min, tdif_max, v_diff)

print("hist_2_uniform.svg");
