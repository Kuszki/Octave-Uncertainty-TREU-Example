...
for i = 1 : nsam
	...
	x1 = x0 + tdif(i) + tdif_mean + gen_randn(step, tdif_std, 's');
	y0 = y1 = zeros(1, step);
...
printf('\\textbf{Case 1)} $\\Delta t_{s} \\sim G(\\num{%0.2f}, \\qty{%0.1e}{s})$, $\\Delta t_{i} \\sim N(\\qty{%0.1e}{s}, \\qty{%0.1e}{s^2})$, $\\delta = \\qty{%0.2f}{\\percent}$ \\vskip6pt \\includegraphics{Pictures/hist_3_gamma_normal}\n', k, theta, tdif_mean, tdif_std^2, v_diff)

print("hist_3_gamma_normal.svg");
