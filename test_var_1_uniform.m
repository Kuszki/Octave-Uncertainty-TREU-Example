...
tdif = (tdif_max-tdif_min)*rand(1, nsam) + tdif_min;
tdif(nsam) = mean(tdif);
...
printf('\\textbf{Case 1)} $\\Delta t_{s} \\sim U(\\qty{%0.1e}{s}, \\qty{%0.1e}{s})$, $\\Delta t_{i} = \\qty{0}{s}$, $\\delta = \\qty{%0.2f}{\\percent}$ \\vskip6pt \\includegraphics{Pictures/hist_1_uniform}\n',  tdif_min, tdif_max, v_diff)

print("hist_1_uniform.svg");
